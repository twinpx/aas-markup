  ...
  Vue.component('form-control-type', {
    data() {
      return {
        ...
        selectedOption: store.state.selectedOptionType ||
    ...
    methods: {
      ...
      clickPagination(e) {
        ...
        if (e.target.getAttribute('href')) {
          ...
          store.commit('changeProp', { prop: 'page', value: 1 * page });
          //render Table
          this.$emit('rendertable');
          //set URL
          this.$emit('seturl');
    ...
    methods: {
      renderTable() {
        (async () => {
          ...
          if (store.state.type) {
            requestObj[window.selectTypeData.name] = store.state.type;
          }
          if (store.state.status) {
            requestObj[window.selectStatusData.name] = store.state.status;
          }
          ...
          const response = await fetch(
            `${window.myScriptURL.getTable}${getQuery(requestObj)}`,
            {
              headers: {
          ...
          if (!response.ok) {
            ...
            //table data
            store.commit('changeProp', { prop: 'tableHtml', value: result });
            //scroll
            window.scrollTo({
              top:
      ...
      seturl() {
        ...
        if (store.state.type) {
          queryObj[window.selectTypeData.name] = store.state.type;
        }
        if (store.state.status) {
          queryObj[window.selectStatusData.name] = store.state.status;
        }
        if (store.state.start) {
