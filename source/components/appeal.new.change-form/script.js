window.onload = function () {
  ...
  Vue.component('formControlTextarea', {
    ...
    <div>
      <div class="row align-items-center">
        <div class="col-lg-6 col-12">
          <div class="b-float-label active" :class="{invalid: isInvalid}">
    ...
    template: `
      <div>
        ...
        <div v-if="formControl.type==='date'">
          <form-control-date v-for="(control, controlIndex) in counter" :formControl="formControl" :fieldsetBlockIndex="control-1" :controlIndex="controlIndex" required="required" @autosave="autosave"></form-control-date>
        </div>
        <div v-else-if="formControl.type==='textarea'">
          <form-control-textarea v-for="(control, controlIndex) in counter" :formControl="formControl" :fieldsetBlockIndex="control-1" :controlIndex="controlIndex" required="required" @autosave="autosave"></form-control-textarea>
        </div>
        <div v-else>
          <form-control v-for="(control, controlIndex) in counter" :formControl="formControl" :fieldsetBlockIndex="control-1" :controlIndex="controlIndex" required="required" @autosave="autosave"></form-control>
        </div>
  ...
  Vue.component('submitButton', {
    ...
    computed: {
      isDisabled() {
        return !(
          store.state.controls.every((control) => !!control.value) &&
          this.checked
        );
      },
    },
