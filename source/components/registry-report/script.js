( function($) {
  ...
  $( function() {
    ...
    if ( urlQuery.field && urlQuery.sort ) {
      var $thElements = $( '.b-registry-report th' );
      ...
      $( '.b-registry-report tbody' ).find( 'tr' ).each( function() {
        $( this ).find( 'td:eq(' + index + ')' ).removeClass( 'asc' ).removeClass( 'desc' ).addClass( urlQuery.sort );
      });
    }

    //tr click
    $( '.b-registry-report' ).delegate( 'tbody tr', 'click', function() {
      window.location = $( this ).data( 'url' );
    });

    //th click, sorting
    $( '.b-registry-report th' ).click( function() {
      var $th = $( this );
      ...
      $.ajax({
        ...
        success: function( json ) {
          ...
          if ( typeof json === 'object' && json.TBODY && json.PAGINATION ) {
            ...
            //set pagination
            var $pagination = $( '.b-registry-report .b-pagination-block' );
